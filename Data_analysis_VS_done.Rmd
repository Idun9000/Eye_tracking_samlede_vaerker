...
```{r}
AmpSaccades <- Saccades %>% 
  group_by(ParticipantID, Task, Trial) %>% 
  summarise(SumAmp = sum(Amplitude))

hist(AmpSaccades$SumAmp)

model <- lmer(SumAmp ~ Task + (1 + Trial|ParticipantID), data = AmpSaccades)
model2 <- lmer(SumAmp ~ Task + (1 + Trial:Task|ParticipantID), data = AmpSaccades)
model3 <-lmer(SumAmp ~ Task + (1|ParticipantID), data = AmpSaccades)
...
```{r}
par(mfrow=c(1,2))
plot(density(Fixations[Fixations$Task == "Searching", ]$Duration), main = "Searching", xlim = c(0, 1000))
plot(density(Fixations[Fixations$Task == "Counting", ]$Duration), main = "Counting", xlim = c(0, 1000))
```
...
mGaus <- lmer(Duration ~ Task + (1|ParticipantID), data = Fixations)

mLog <- glmer(Duration ~ Task + (1|ParticipantID), family = gaussian(link="log"), data = Fixations)

...
```{r load data}
Fixations <- read_csv("Data/Visual Search/Fixations_VS.csv")
Saccades <- read_csv("Data/Visual Search/Fixations_VS.csv")
```
...
Pictures <- Pictures %>% 
  rename(Task = SearchOrCount) %>% 
  mutate(Task = case_when(Task == "Search" ~ "Searching", Task == "Count" ~ "Counting"))

Fixations <- right_join(Pictures, Fixations)

Saccades <- right_join(Pictures, Saccades)
```
...
Saccades <- Saccades %>% mutate(Position = 1050-PositionY)

hist(Fixations$PositionY)
hist(Saccades$PositionY)
```
...
```{r}
make_scanpath("elisius1", 5)
```

And here is one from the Count condition:

```{r}
make_scanpath("elisius1", 10)
```
...
```{r}
make_scanpath_all <- function(participant){
  for(i in 1:10){
    make_scanpath(participant, i)
  }
}

make_scanpath_all("elisius1")
```
...
```{r}
make_heatmap("elisius1", 6)
```

And one more:

```{r}
make_heatmap("elisius1", 3)
```
...
```{r}
make_heatmap_facet <- function(trial){
...
```{r}
make_heatmap_cumulative <- function(trial){
  x <- Fixations[Fixations$Trial == trial,]
...
```{r}
Delphine_Heatmaps <- make_heatmap_facet(7)
Delphine_Heatmaps

# ggsave("Delphine_Heatmaps.png")
```
...
```{r}
make_heatmap_cumulative(7)

AOIDOLPHIN = c(450,950,250,500)

xDolphin <-
  Fixations %>% filter(PositionX >= AOIDOLPHIN[1] &
                         PositionX <= AOIDOLPHIN[2] &
                         PositionY >= AOIDOLPHIN[3] & PositionY <= AOIDOLPHIN[4])
```
...
ggplot(xDolphin, aes(PositionX, PositionY, color = FixationNo)) + 
  annotation_custom(img, xmin = 0, xmax = 1680, ymin = 0, ymax = 1050) +
  # this line draws the rectangle for the AOI
  geom_rect(xmin=AOIDOLPHIN[1], xmax=AOIDOLPHIN[2], ymin=AOIDOLPHIN[3], ymax=AOIDOLPHIN[4], fill = NA, size = 1, color = 'red') +
  stat_density2d(geom = "raster", aes(fill = ..density.., alpha = sqrt(sqrt(..density..))), contour = FALSE, n = 1000) + 
  scale_fill_gradientn(colours = heat_colors(10), trans="sqrt") +
  scale_alpha(range = c(0.1, 0.6)) +
  xlim(0,1680) + ylim(0,1050) +
  theme(legend.position = "none") +facet_wrap(~Task)
```
...
x2 <- Fixations %>%
  ...
  mutate(
    ...
    InAOI3 = ifelse(
      PositionX >= AOI3[1] &
        PositionX <= AOI3[2] &
        PositionY >= AOI3[3] & PositionY <= AOI3[4],
      TRUE,
      FALSE
    )
  )
...
ggplot(x2, aes(PositionX, PositionY, color = FixationNo)) + 
  annotation_custom(img, xmin = 0, xmax = 1680, ymin = 0, ymax = 1050) +
  geom_rect(xmin=AOI1[1], xmax=AOI1[2], ymin=AOI1[3], ymax=AOI1[4], fill = NA, size = 1, color = 'red') +
  annotate(geom = "label", x = 500, y = 450, label = "Birds", color = "red") +
  geom_rect(xmin=AOI2[1], xmax=AOI2[2], ymin=AOI2[3], ymax=AOI2[4], fill = NA, size = 1, color = 'blue')+
  annotate(geom = "label", x = 850, y = 750, label = "More birds", color = "blue") +
  geom_rect(xmin=AOI3[1], xmax=AOI3[2], ymin=AOI3[3], ymax=AOI3[4], fill = NA, size = 1, color = 'yellow') +
  annotate(geom = "label", x = 1500, y = 400, label = "Even more birds", color = "yellow")+
  xlim(0,1680) + ylim(0,1050) +
  theme(legend.position = "none") +
  theme_light()
```
...
ggplot(Prop2, aes(FixationNo, Proportion, color = AOI)) +
  geom_smooth() + ylim(-10,100) +
  theme_light() + facet_wrap(~Task)
```
...
```{r}
# Investigating the time it takes to reach the AOI for both conditions
x <-
  Fixations %>% filter(PositionX >= AOI[1] &
                         PositionX <= AOI[2] &
                         PositionY >= AOI[3] & PositionY <= AOI[4])
...
x %>%
  filter(Stimulus == "sheep.jpg") %>% 
  group_by(ParticipantID, Task) %>% 
  slice_min(FixationNo) %>% 
  ungroup() %>% 
  group_by(Task) %>% 
  summarize(mean(FixationNo), sd(FixationNo))
